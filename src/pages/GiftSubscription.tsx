  ...
  return (
    <div className="flex min-h-dvh items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        <div className="border-dark-600 border-t-accent-500 h-8 w-8 animate-spin rounded-full border-2" />
      </div>
    </div>
  );
}
    ...
    <div className="flex min-h-dvh items-center justify-center px-4">
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <div className="bg-error-500/10 flex h-16 w-16 items-center justify-center rounded-full">
          <WarningCircleIcon className="text-error-400 h-8 w-8" />
        </div>
        <h2 className="text-dark-50 text-lg font-semibold">{t('gift.failedTitle')}</h2>
...
function DisabledState() {
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center px-4">
      <div className="flex max-w-sm flex-col items-center gap-4 text-center">
        <div className="bg-dark-800/50 flex h-16 w-16 items-center justify-center rounded-full">
          <BanIcon className="text-dark-400 h-8 w-8" />
        </div>
        <h2 className="text-dark-50 text-lg font-semibold">{t('gift.featureDisabled')}</h2>
        <p className="text-dark-300 text-sm">{t('gift.redirecting')}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-dark-50 text-base font-bold">{tariff.name}</p>
        <p
          className={cn(
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected
          ? 'from-accent-500 to-accent-600 shadow-accent-500/25 bg-linear-to-r text-white shadow-lg'
        ...
        {hasDiscount && period.discount_percent != null && (
          <span
            className={cn(
              ...
              isSelected ? 'text-on-accent bg-white/20' : 'bg-accent-500/20 text-accent-400',
            )}
          >
...
function PaymentModeToggle({
  ...
  return (
    <div
      ...
      className="bg-dark-800/50 flex rounded-xl p-1"
    >
      <button
        type="button"
...
function PaymentMethodCard({
  ...
  return (
    ...
    >
      ...
      >
        {method.icon_url && (
          <div className="bg-dark-800/50 flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl">
            <img src={method.icon_url} alt="" className="h-6 w-6 object-contain" />
          </div>
        )}
        <div className="min-w-0 flex-1">
          <p className="text-dark-100 text-sm font-medium">{method.display_name}</p>
          {method.description && (
            <p className="text-dark-400 mt-0.5 truncate text-xs">{method.description}</p>
          )}
        </div>
        <div
          className={cn(
      ...
      {isSelected && hasSubOptions && (
        <div className="border-dark-800/30 border-t px-4 pt-3 pb-4">
              ...
              <button
                ...
                className={cn(
                  ...
                  selectedSubOption === opt.id
                    ? 'bg-accent-500 text-on-accent shadow-accent-500/25 shadow-sm'
...
function BuyTabContent({
  ...
  return (
    <div className="space-y-6">
      ...
      {showTariffCards && (
        <div>
          <h2 className="text-dark-400 mb-3 text-xs font-semibold tracking-wider uppercase">
            {t('gift.selectTariff')}
          </h2>
      ...
      {selectedTariff?.description && (
        <div className="border-dark-800/30 bg-dark-800/20 rounded-xl border px-4 py-3">
          <p className="text-dark-300 text-sm">{selectedTariff.description}</p>
        </div>
      )}
      ...
      {config.promo_group_name && (
        <div className="border-success-500/30 bg-success-500/10 flex items-center gap-3 rounded-xl border p-3">
          <div className="bg-success-500/20 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
            <SparklesIcon className="text-success-400 h-4 w-4" />
          </div>
          <div>
      ...
      {config.active_discount_percent != null && config.active_discount_percent > 0 && (
        <div className="border-warning-500/30 bg-warning-500/10 flex items-center gap-3 rounded-xl border p-3">
          <div className="bg-warning-500/20 flex h-8 w-8 shrink-0 items-center justify-center rounded-full">
            <SparklesIcon className="text-warning-400 h-4 w-4" />
          </div>
      ...
      {paymentMode === 'balance' && (
        <div className="border-dark-800/50 bg-dark-900/50 rounded-2xl border p-4">
          <div className="flex items-center justify-between">
            <span className="text-dark-400 text-sm">{t('gift.yourBalance')}</span>
        ...
        {insufficientBalance && (
          ...
          >
            <p className="text-warning-400 text-sm">
              ...
              <Link
                to="/balance"
                className="text-accent-400 font-medium underline underline-offset-2"
              >
                {t('gift.topUpBalance')}
              </Link>
            </p>
      ...
      <button
        ...
        className={cn(
          ...
          canSubmit && !purchaseMutation.isPending
            ? 'bg-accent-500 text-on-accent shadow-accent-500/25 hover:bg-accent-400 hover:shadow-accent-500/40 shadow-lg active:scale-[0.98]'
  ...
  if (activateMutation.isSuccess && activateMutation.data) {
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-12 text-center">
        <div className="bg-accent-500/20 flex h-16 w-16 items-center justify-center rounded-full">
          <CheckCircleIcon className="text-accent-400 h-8 w-8" />
        </div>
        <h2 className="text-dark-50 text-xl font-bold">{t('gift.activateSuccess')}</h2>
  ...
  return (
    <div className="flex flex-col items-center gap-6 py-8">
      ...
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="bg-accent-500/20 flex h-16 w-16 items-center justify-center rounded-full">
          <KeyIcon className="text-accent-400 h-8 w-8" />
        </div>
        <h2 className="text-dark-50 text-xl font-bold">{t('gift.activateTitle')}</h2>
        <p className="text-dark-400 max-w-xs text-sm">{t('gift.activateDescription')}</p>
      </div>
      ...
      <div className="w-full max-w-sm">
        <input
          ...
          className="border-dark-700/50 bg-dark-800/50 text-dark-50 placeholder-dark-500 focus:border-accent-500/50 focus:ring-accent-500/25 w-full rounded-2xl border px-6 py-4 text-center font-mono text-sm transition-colors outline-none focus:ring-1"
      ...
      <button
        ...
        className={cn(
          ...
          code.trim() && !activateMutation.isPending
            ? 'bg-accent-500 text-on-accent shadow-accent-500/25 hover:bg-accent-400 shadow-lg active:scale-[0.98]'
            : 'bg-dark-800 text-dark-500 cursor-not-allowed',
        )}
      >
...
function CopiedToast({ onDismiss }: { onDismiss: () => void }) {
  ...
  return (
    ...
    >
      <div className="border-dark-700/50 bg-dark-900/95 flex items-center gap-2 rounded-full border px-5 py-2.5 shadow-2xl shadow-black/40 backdrop-blur-md">
  ...
  return (
    <div className="border-dark-800/50 bg-dark-900/50 rounded-2xl border p-4">
      ...
      <div className="mb-3 flex items-start justify-between">
        <h3 className="text-dark-50 text-base font-bold">{gift.tariff_name ?? t('gift.tariff')}</h3>
        <span
          className={cn(
      ...
      {/* Info line */}
      <p className="text-dark-400 mb-3 text-xs">
      ...
      {!isActivated && (
        <>
          {/* Gift code display */}
          <div className="bg-dark-800/80 mb-3 rounded-xl px-4 py-4 text-center">
            <p className="text-accent-400 font-mono text-base font-bold tracking-[0.15em]">
              {giftCode}
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={handleShare}
            className="bg-accent-500 text-on-accent hover:bg-accent-400 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-bold tracking-wider uppercase transition-colors active:scale-[0.98]"
          >
...
function ReceivedGiftCard({ gift }: { gift: ReceivedGift }) {
  ...
  return (
    <div className="border-dark-800/50 bg-dark-900/50 rounded-2xl border p-4">
      ...
      <div className="mb-3 flex items-start justify-between">
        <h3 className="text-dark-50 text-base font-bold">{gift.tariff_name ?? t('gift.tariff')}</h3>
        <span className="bg-dark-700 text-dark-400 rounded-lg px-2.5 py-1 text-xs font-bold">
          {statusText}
        </span>
      </div>

      {/* Info line */}
      <p className="text-dark-400 mb-2 text-xs">
        {formatGiftDate(gift.created_at)}
        {' \u2022 '}
        {gift.period_days} {t('gift.daysShort')}
        {' \u2022 '}
      ...
      {/* Gift message */}
      {gift.gift_message && (
        <div className="bg-dark-800/50 mt-2 rounded-xl p-3">
          <p className="text-dark-300 text-xs italic">{gift.gift_message}</p>
        </div>
      )}
    </div>
  );
...
function MyGiftsTabContent() {
  ...
  if (isEmpty) {
    return (
      <div className="flex flex-col items-center gap-4 py-12 text-center">
        <div className="bg-dark-800/50 flex h-16 w-16 items-center justify-center rounded-full">
          <InboxIcon className="text-dark-400 h-8 w-8" />
        </div>
        <h2 className="text-dark-200 text-lg font-semibold">{t('gift.myGiftsEmpty')}</h2>
        <p className="text-dark-400 max-w-xs text-sm">{t('gift.myGiftsEmptyDesc')}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-dvh">
      <div className="mx-auto max-w-2xl px-4 py-6">
        ...
        >
          <div className="bg-accent-500/20 flex h-10 w-10 items-center justify-center rounded-full">
            <GiftIcon className="text-accent-400 h-5 w-5" />
          </div>
          <h1 className="text-dark-50 text-2xl font-bold">{t('gift.pageTitle')}</h1>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-dark-800/50 mb-6 rounded-2xl p-1"
        >
